...
public class LoginViewController : MonoBehaviour
{
    ...
    public void OnLogin()
    {
        ...
        ArxoftLoginSaltRequest saltRequest = new ArxoftLoginSaltRequest().WithUserEmail(email);
        Dictionary<string, string> headers = new Dictionary<string, string>();
        headers.Add("Content-Type", "application/json");
        byte[] data = System.Text.Encoding.ASCII.GetBytes(saltRequest.ToJson().ToCharArray());
        WWW apiCall = new WWW(URLStorage.ARXOFT_USER_LOGIN_SALT, data, headers);
        LoadingWheel.StarAnimation();
        StartCoroutine(OnGetSaltResult(apiCall));
        Debug.Log(saltRequest.ToJson());
    ...
    IEnumerator OnGetSaltResult(WWW response)
    {
        yield return response;

        if (response.error != null)
        {
            Debug.Log("ERROR AT REQUEST! Error: " + response.error + response.text);
